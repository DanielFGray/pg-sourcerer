...
export interface PgIndex {
  ...
  /**
   * Resolved operator class names for each indexed column (e.g., "gin_trgm_ops", "tsvector_ops").
   * This is computed by joining pg_opclass during introspection.
   */
  indclassnames: ReadonlyArray<string> | null;
}
  ...
  indexes as (
    select *,
      (select array_agg(opc.opcname order by u.ord)
       from unnest(pg_index.indclass) with ordinality as u(oid, ord)
       join pg_catalog.pg_opclass opc on opc.oid = u.oid
      ) as indclassnames
