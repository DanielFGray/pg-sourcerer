...
describe("parseSmartTags", () => {
  describe("valid formats with sourcerer key", () => {
    ...
    it.effect("parses omit as array of shape kinds", () =>
      Effect.gen(function* () {
        const result = yield* parseSmartTags('{"sourcerer": {"omit": ["insert", "update"]}}', ctx);
  ...
  describe("errors", () => {
    it.effect("fails on malformed JSON", () =>
      Effect.gen(function* () {
        const result = yield* parseSmartTags("{malformed json}", ctx).pipe(Effect.either);
    ...
    it.effect("fails on unclosed brace", () =>
      Effect.gen(function* () {
        const result = yield* parseSmartTags('{"sourcerer": {}', ctx).pipe(Effect.either);
        expect(result._tag).toBe("Left");
        if (result._tag === "Left") {
          expect(result.left).toBeInstanceOf(TagParseError);
        }
    ...
    it.effect("fails on invalid omit value", () =>
      Effect.gen(function* () {
        const result = yield* parseSmartTags('{"sourcerer": {"omit": "invalid"}}', ctx).pipe(
          Effect.either,
        );
        expect(result._tag).toBe("Left");
        if (result._tag === "Left") {
    ...
    it.effect("fails on invalid deprecated value", () =>
      Effect.gen(function* () {
        const result = yield* parseSmartTags('{"sourcerer": {"deprecated": 123}}', ctx).pipe(
          Effect.either,
        );
        expect(result._tag).toBe("Left");
      }),
