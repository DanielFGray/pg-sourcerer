  ...
  describe("entity generation", () => {
    it.effect("generates interfaces for Post entity", () =>
      Effect.gen(function* () {
        ...
        yield* typesPlugin.plugin.run({ outputDir: "types" }).pipe(Effect.provide(testLayer));
  ...
  describe("configuration", () => {
    it.effect("uses outputDir from config", () =>
      Effect.gen(function* () {
        ...
        yield* typesPlugin.plugin.run({ outputDir: "custom/path" }).pipe(Effect.provide(testLayer));
    ...
    it.effect("overrides type with tsType hint by table and column", () =>
      Effect.gen(function* () {
        ...
        const otherFiles = all.filter(
          e => !e.path.includes("User.ts") && !e.path.includes("enums.ts"),
        );
